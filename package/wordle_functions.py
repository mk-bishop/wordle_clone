...
class EnrichedText():
    ...
    def pretty_string(self):
        return f"[{self.style}]{self.string}[/{self.style}]"
...
def guess(word_list, answer):
    ...
    for i in range(len(turn_list)): #max num of turns
        ...
        for x in range(len(answer)):
            ...
            # inspect(turn[4], all=True)
        
        #the colors aren't working with the f-string but everything else is
        #I thought it had to do with rich reading the brackets as markup but attempting to escape broke it more.
        #the colors work if you print immediately after assigning the variables, but not in the ascii board.
        #the rich library has a really cool table format, so I may try again on wednesday to make that version, and maybe then the colors will work. as long as I'm learning a whole library, might as well, right?
